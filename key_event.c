...
__attribute__((nonnull))
static void key_move(struct JBWMClient *  c, const uint8_t flags)
{
...
static void warp_to(struct JBWMClient *  c) {
    point(c, 0, 0);
    ...
    XKeyEvent * e)
{
    ...
    case JBWM_KEY_NEXT:
        if(*(s->current_client))
            next(*(s->current_client), (s->current_client),
                s->vdesk);
        break;
    case XK_0:
        opt.zero = true;
    ...
    default:
        if (target)
            handle_client_key_event((s->current_client), opt.mod, key);
    }
}
