...
#define JBWM_VWM_CHANGE_PROPERTY(c, property, state, n)\
        XChangeProperty(jbwm.X.dpy, c->window,\
		JBWM_ATOM_VWM_DESKTOP, property, 32,\
                PropModeReplace, state, n);

#define JBWM_UPDATE_NET_WM_DESKTOP(c)\
	JBWM_VWM_CHANGE_PROPERTY(c, XA_CARDINAL, (unsigned char *)&c->vdesk, 1);
